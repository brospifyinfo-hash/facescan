...
//
// SUPPORT_TO_EMAIL HAS NO DEFAULT ON PURPOSE
//
// This repository is public. A destination address baked into the source is
// an address handed to every scraper that walks it, and the inbox that
// receives support mail is the last one that can afford that. Unset, the
// route reports "unconfigured" and the form says so.
//
...
export async function sendSupportMessage(
  ...
  if (!key || !to) {
    if (process.env.NODE_ENV !== "production") {
      console.info(
        `[support] no ${!key ? "RESEND_API_KEY" : "SUPPORT_TO_EMAIL"} — message from ${msg.email}:\n${msg.body}`,
      );
